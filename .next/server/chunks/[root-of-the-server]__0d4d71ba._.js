module.exports = [
"[project]/.next-internal/server/app/api/admin/batches/route/actions.js [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/src/app/api/admin/batches/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(_request) {
    try {
        ...
        const batches = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$db$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["db"].userBatch.findMany({
            orderBy: {
                createdAt: 'desc'
            },
            include: {
        ...
        const activeBatches = batches.filter((b)=>b._count.users > 0);
        const result = activeBatches.map((batch)=>{
            const allAssessments = batch.users.flatMap((u)=>u.assessments);
            const completed = allAssessments.filter((a)=>a.status === 'COMPLETED');
            const avgScore = completed.length > 0 ? (completed.reduce((s, a)=>s + a.score, 0) / completed.length).toFixed(1) : null;
            const avgPct = completed.length > 0 ? (completed.reduce((s, a)=>s + a.percentage, 0) / completed.length).toFixed(1) : null;
            return {
                id: batch.id,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0d4d71ba._.js.map